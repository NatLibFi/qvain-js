		...
		if (restProps.length > 0) {
			...
			if (allowAddProps) {
				for (let i in restProps) {
					...
					if (!recurse(deepcopy(addPropSchema), data[prop], out, data, path + '/' + prop)) addError(origSchema, "failed prop: " + prop)
				}
			} else {
...
function validateArray(schema, data, out, parent, path, recurse) {
	...
	if (!asTuple) {
		...
		for (let i in data) {
			...
			if (!recurse(itemSchema, data[i], out, data, path + '/' + i, recurse)) addError(schema, "list validation failed for item: " + i)
		...
		if (!allowAddItems && schema['items'].length < data.length) {
			addError(schema, "additional items not allowed in array")
			//return checkValid(schema)
		}
		...
		for (let i in data) {
			...
			*/
			...
			if (!recurse(itemSchema, data[i], out, data, path + '/' + i, recurse)) addError(schema, "tuple validation failed for index: " + i)
		}
	}

	// minItems and maxItems
	if (typeof schema['minItems'] === 'number' && data.length < schema['minItems']) addError(schema, "too few items in array")
	if (typeof schema['maxItems'] === 'number' && data.length > schema['maxItems']) addError(schema, "too many items in array")
	...
	if (schema['uniqueItems'] === true) {
		...
		let shittySet = {}
		if (data.some(e => e in shittySet || shittySet[e]++ )) addError(schema, "list contains duplicates")
	}
...
function validateEnum(schema, data, out, parent, path, recurse) {
	...
	if (schema['enum'].length <= 0) { addError(schema, "value not in enum") }
	
	if (!schema.enum.some(el => deepequal(el, data))) addError(schema, "value not in enum")
