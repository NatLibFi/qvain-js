	...
	<div>
		...
		<b-modal id="addPersonModal" title="Add a person" size="lg" centered>
			<b-container fluid class="px-0">
				...
				first name(s): <b-input v-model="newFirst"></b-input>
				last name(s): <b-input v-model="newLast"></b-input>
				name: <b-input v-model="people[editingIndex].name"></b-input>
				email: <b-input v-model="people[editingIndex].email"></b-input>
				</b-form-group>

				will be shown as: <span class="text-muted">{{ lastFirstName }}</span><br/>

				<div v-if="editingIndex !== null">
					{{ people[this.editingIndex].name }}
					{{ people[this.editingIndex].email }}
				</div>
				<br/>



				<b-btn @click="add">add</b-btn> <b-btn>cancel</b-btn>
			</b-container>
		</b-modal>
		...
		<b-card no-body style="max-width: 60rem;" header-class="">
			...
			<b-navbar slot="header" class="m-0 p-0" toggleable>
				<b-nav-text><h5>Persons</h5></b-nav-text>
			...
			<b-list-group flush>
				<b-list-group-item v-for="(person, i) in people" :key="i">
					...
					<b-media class="mt-2">
					<i class="fas fa-user fa-fw fa-lg" slot="aside"></i>
					<b-button-group class="float-right">
						<b-btn :id="`btn-edit-${i}`" variant="dark" @click="open(i)"><i class="fas fa-user-edit fa-fw text-light"></i></b-btn>
						<b-btn :id="`btn-delete-${i}`" variant="danger" @click="remove(i)"><i class="fas fa-trash-alt fa-fw text-light"></i></b-btn>
					</b-button-group>
					...
					<div class="d-flex w-100 justify-content-between my-2">
						<h5 class="mb-1"><i class="fas fa-user fa-fw"></i>  {{ person.name }}</h5>
			...
			<b-card-body v-if="people.length < 1">
				<p class="card-text"><i>add a person</i></p>
			</b-card-body>
...
export default {
	...
	computed: {
		state () {
			...
			return 'name' in this.people[this.editingIndex] && (this.people[this.editingIndex].name.length > 1 && this.people[this.editingIndex].name.length <= 30) ? true : false
		},
		invalidFeedback () {
