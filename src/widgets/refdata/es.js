...
export default function esApiClient(index, doctype) {
	return axios.get(
		`${apiUrl}/${index}/${doctype}/_search?size=10&pretty=1&filter_path=hits.hits._source`, {
			timeout: 5000,
			responseType: 'json',
		})
}
...
export function esApiSearchClient(index, doctype, searchterm) {
	return axios.get(
		`${apiUrl}/${index}/${doctype}/_search?q=${searchterm}&size=10&pretty=1&filter_path=hits.hits._source`, {
			timeout: 5000,
			responseType: 'json',
		})
}
