<template>
	<div>
		...
		<!-- actual component -->
		<!-- keep-alive -->
		<component v-if="activeTab === myTab" :class="[ activeTab === myTab ? 'righttab': 'wrongtab']"
		  :is="widget" v-bind="widgetProps" :schema="schema" :path="path" :value="parent[property]"
		  :valtype="dataType" :parent="parent" :property="property" :tab="myTab" :activeTab="activeTab"
		  :depth="newdepth" v-on="$listeners">
			<p>{{ dataType }}</p>
		</component>
		<skip v-else :schema="schema" :path="path" :value="parent[property]" :valtype="dataType"
		  :parent="parent" :property="property" :tab="myTab" :activeTab="activeTab" :depth="depth"
		  v-on="$listeners"></skip>
	</div>
...
export default {
  ...
  methods: {
    ...
    handleCombiners: function() {
      for (let i in COMBINERS) {
        ...
        if (
          typeof this.schema[combiner] === 'object' &&
          this.schema[combiner] instanceof Array
        ) {
    ...
    defaultWidget: function(schemaType) {
      ...
      switch (schemaType) {
        ...
        case undefined:
          console.log(
            'schemaType: checking for combiners...',
            this.handleCombiners(),
          )
  ...
  computed: {
    ...
    widget: function() {
      console.log(
        `widget chain for ${this.path}:`,
        this.selectedWidget,
        this.uiForSchema['widget'],
        this.uiForDef['widget'],
        this.defaultWidget(this.dataType),
      )
      return (
        this.selectedWidget ||
        this.uiForSchema.widget ||
    ...
    ui: function() {
      ...
      if (this.schema['$deref']) {
        return Object.assign(
          {},
          this.$store.state.hints[this.schema['$deref']],
          this.$store.getters.uiForPath(this.path),
        )
      }
  ...
  created() {
    console.log(
      'schema-tab-selector(',
      this.path,
      '): calling setDataType (created) with',
      this.schema['type'],
      'and calling vivicate()',
    )
    this.setDataType(this.schema['type'])
